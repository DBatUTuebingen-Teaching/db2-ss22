...
\set a 42   -- parameter 'a' specified in web form
\set c NULL -- parameter 'c' left unspecified (= wild card: any 'c' value is fine)
...
DROP TABLE IF EXISTS ternary_2M;
CREATE TABLE ternary_2M (a int NOT NULL, b text NOT NULL, c float);

INSERT INTO ternary_2M(a, b, c)
  SELECT i            AS a,
         md5(i::text) AS b,
         log(i)       AS c
  FROM   generate_series(1, 2000000, 1) AS i;
